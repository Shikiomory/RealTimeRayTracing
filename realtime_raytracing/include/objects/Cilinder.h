...
public:
	...
	bool hit(const Ray& r, Interval t_int, hit_record& rec) const override {
		...
		float l_or_y = local_origin.y;
		local_origin.y = 0.0f;
		Vector3 local_dir = r.get_direction();
		float l_dir_y = local_dir.y;
		local_dir.y = 0.0f;

		float ak = local_dir.squareLength();
		float h = dot(local_dir, local_origin);
		float ck = local_origin.squareLength() - radius * radius;

		float discriminant = h * h - ak * ck;

		float y_max = std::max(pos1.y, pos2.y);
		float y_min = std::min(pos1.y, pos2.y);
